<?php
/**
 * Affiliate Link WordPress Plugin API export via XML-RPC
 *
 ...
 */
function affli_xmlrpc_api_version($args)
{
  $username = $args[0];
  $password = $args[1];
 ...
 */
function affli_xmlrpc_create_pretty_link( $args )
{
  $username = $args[0];
  $password = $args[1];
  ...
  if( $link = affli_create_pretty_link( $target_url, 
                                       $slug, 
                                       $name, 
                                       $description, 
                                       $group_id, 
    ...
    return new IXR_Error( 401, __( 'There was an error creating your Link' ) );
}

function affli_xmlrpc_update_pretty_link( $args )
{
  $username = $args[0];
  $password = $args[1];
  ...
  if( $link = affli_update_pretty_link( $id, 
                                       $target_url, 
                                       $slug, 
                                       $name, 
                                       $description, 
 ...
 *                                         link groups.
 */
function affli_xmlrpc_get_all_groups($args)
{
  $username = $args[0];
  $password = $args[1];
  ...
  if( $groups = affli_get_all_groups())
    return $groups;
  else
    return new IXR_Error( 401, __( 'There was an error fetching the Link Groups' ) );
}
 ...
 *                                         links.
 */
function affli_xmlrpc_get_all_links($args)
{
  $username = $args[0];
  $password = $args[1];
  ...
  if( $links = affli_get_all_links())
    return $links;
  else
    return new IXR_Error( 401, __( 'There was an error fetching the Links' ) );
}
 ...
 */
function affli_xmlrpc_get_link_from_slug($args)
{
  $username = $args[0];
  $password = $args[1];
  ...
  if( $link = affli_get_link_from_slug($slug) )
    return $link;
  else
    return new IXR_Error( 401, __( 'There was an error fetching your Link' ) );
}
 ...
 *                                         data about the given pretty link.
 */
function affli_xmlrpc_get_link($args)
{
  $username = $args[0];
  $password = $args[1];
 ...
 */
function affli_xmlrpc_get_pretty_link_url($args)
{
  $username = $args[0];
  $password = $args[1];
  ...
  if( $url = affli_get_pretty_link_url($id) )
    return $url;
  else
    return new IXR_Error( 401, __( 'There was an error fetching your Link URL' ) );
}
z?>
