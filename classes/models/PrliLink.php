...
class PrliLink
{
    ...
    {
      global $wpdb;
      $this->table_name = "{$wpdb->prefix}affli_links";
    }

    function create( $values )
    {
      global $wpdb, $affli_url_utils;

      $values['name'] = (!empty($values['name'])?$values['name']:$affli_url_utils->get_title($values['url'],$values['slug']));
      ...
      $query = $wpdb->prepare( $query_str,
                               $values['url'],
                               str_replace(array(" ","+"),array("-","-"),$values['slug']),
                               $values['name'],
    ...
    function destroy( $id )
    {
      ...
      global $wpdb, $affli_click;

      $reset = 'DELETE FROM ' . $affli_click->table_name .  ' WHERE link_id=' . $id;
    ...
    function getOneFromSlug( $slug, $return_type = OBJECT, $include_stats = false )
    {
      ...
      if($include_stats)
        $query = 'SELECT li.*, ' .
                    '(SELECT COUNT(*) FROM ' . $affli_click->table_name . ' cl ' .
                        'WHERE cl.link_id = li.id' . $affli_click->get_exclude_where_clause( ' AND' ) . ') as clicks, ' .
                    '(SELECT COUNT(*) FROM ' . $affli_click->table_name . ' cl ' .
                        'WHERE cl.link_id = li.id ' .
                        'AND cl.first_click <> 0' . $affli_click->get_exclude_where_clause( ' AND' ) . ') as uniques ' .
    ...
    {
      global $wpdb, $affli_click, $affli_group, $affli_utils;
      ...
      if($include_stats)
        $query = 'SELECT li.*, ' .
                    ...
                    '(SELECT COUNT(*) FROM ' . $affli_click->table_name . ' cl ' .
                        ...
                        'AND cl.first_click <> 0' . $affli_click->get_exclude_where_clause( ' AND' ) . ') as uniques ' .
                 'FROM '. $this->table_name . ' li ' .
                 $affli_utils->prepend_and_or_where(' WHERE', $where) . $order_by;
      else
    ...
    function getRecordCount($where="")
    {
      ...
      $query = 'SELECT COUNT(*) FROM ' . $this->table_name . ' li' . $affli_utils->prepend_and_or_where(' WHERE', $where);
      return $wpdb->get_var($query);
    }
    ...
    {
      global $wpdb, $affli_click, $affli_utils, $affli_group;
    ...
    function generateValidSlug($num_chars = 3)
    {
      ...
      while( in_array($slug,$slugs) or !$affli_utils->slugIsAvailable($slug) )
        $slug = base_convert( rand($min_slug_value,$max_slug_value), 10, 36 );
    ...
    function get_pretty_link_url($slug)
    {
      ...
      if((isset($link->param_forwarding) and $link->param_forwarding == 'custom') and
        ...
        return "&lt;img src=\"".$affli_blogurl . '/' . $link->slug . $link->param_struct . "\" width=\"1\" height=\"1\" style=\"display: none\" /&gt;";
      else if((!isset($link->param_forwarding) or $link->param_forwarding != 'custom') and
              (isset($link->redirect_type) and $link->redirect_type == 'pixel'))
        return "&lt;img src=\"".$affli_blogurl . '/' . $link->slug . "\" width=\"1\" height=\"1\" style=\"display: none\" /&gt;";
        ...
        return $affli_blogurl . '/' . $link->slug . $link->param_struct;
      else
        return $affli_blogurl . '/' . $link->slug;
    }
    ...
    {
      global $wpdb, $affli_utils, $affli_blogurl;
      ...
      if( $values['url'] == "$affli_blogurl/".$values['slug'] )
      ...
      if( preg_match('/^[\?\&\#]+$/', $values['slug'] ) )
        $errors[] = "Link must not contain question marks, ampersands or number signs.";

      if( !$affli_utils->slugIsAvailable($values['slug'],$values['id']) )
        $errors[] = "This link slug is already taken, please choose a different one";
